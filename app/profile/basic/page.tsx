...
export default function BasicInfoPage() {
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      ...
      try {
        ...
        if (response.status === 401) { router.replace('/login?redirect=%2Fprofile%2Fbasic'); return }
  ...
  if (completedGender) return <CompletionChoice gender={completedGender} />

  return <div className="mx-auto flex min-h-screen w-full max-w-xl flex-col px-5 pb-28 pt-8">
    <header className="mb-7"><p className="text-xs font-black text-boot-primary">QUANTUM · 최소 가입</p><h1 className="mt-2 text-2xl font-black text-boot-ink">커뮤니티에 들어갈 준비를 해요</h1><p className="mt-2 text-sm leading-6 text-boot-muted">휴대폰 인증과 꼭 필요한 정보만 저장해요. 월드컵과 사진은 가입 뒤 선택할 수 있어요.</p></header>
...
function CompletionChoice({ gender }: { gender: CommunityGender }) {
