      ...
      <div className="mx-auto w-full max-w-4xl px-4 pt-5 sm:px-6 sm:pt-7">
        ...
        {loading ? <ProfileSkeleton /> : (
          <>
            ...
            <Link href="/account" className="flex min-h-14 items-center justify-between border-b border-boot-hairline px-1 text-sm font-black">
              <span>계정·개인정보·이용약관</span><ChevronRight size={18} className="text-boot-muted" />
            </Link>

            <div className="py-6">
              {!showConfirm ? (
                <button type="button" onClick={() => setShowConfirm(true)} className="flex min-h-11 w-full items-center justify-center gap-2 text-xs font-black text-[#B44236]"><AlertTriangle size={16} /> 프로필 사진 초기화</button>
              ) : (
                <div className="rounded-lg border border-[#F1B4AD] bg-white p-4">
                  <p className="text-sm font-black">업로드한 프로필 사진을 모두 지울까요?</p>
                  <p className="mt-1 text-xs font-bold text-boot-muted">사진만 삭제되며 기본정보·친구·모임·결제 기록은 유지돼요. 완료 후 기본정보 입력 화면으로 이동합니다.</p>
                  <div className="mt-4 grid grid-cols-2 gap-2">
                    ...
                    <button type="button" onClick={() => void handleReset()} disabled={resetting} className="min-h-11 rounded-lg bg-[#B44236] text-sm font-black text-white disabled:opacity-50">{resetting ? '사진 삭제 중...' : '사진 삭제'}</button>
                  </div>
                </div>
              )}
            </div>
