...
export default async function GuidedParticipationPreviewPage({ searchParams }: {
  searchParams: Promise<{ scene?: string | string[]; ranking?: string | string[]; flow?: string | string[]; sport?: string | string[] }>
}) {
  if (process.env.NODE_ENV !== 'development' || process.env.QUANTUM_LOCAL_RUNTIME_MODE !== 'offline-ui') notFound()
  const { scene, ranking, flow, sport } = await searchParams
  ...
  return <GuidedParticipationPreview scene={selected} ranking={rankingExample} flow={flow==='invites'||flow==='recruitment'?flow:'league'} sport={isLeagueSport(sport)?sport:'lol'} />
}
