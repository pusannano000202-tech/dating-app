...
export async function POST(request: NextRequest) {
  ...
  const { data, error } = await supabase.rpc('decline_quantum_event_room_invite', {
    p_token: token,
  })
  if (error) {
    ...
    return jsonError('room_invite_decline_failed', 500)
  }

  return NextResponse.json({ declined: data === true })
}
