...
export async function POST(req: NextRequest) {
  try { assertTrustedMutationOrigin(req) } catch (error) {
    return noStoreError('request_not_allowed', error instanceof TrustedOriginError ? error.status : 403)
  }
  const supabase = createSupabaseRequestClient(req)
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) {
  ...
  if (!receiverUserId && receiverNickname) {
    ...
    if (requestError) {
      if (requestError.code === '40001' || requestError.message.includes('friend_pair_retryable')) {
        return NextResponse.json({ error: 'friend_pair_retryable' }, { status: 409 })
      }
  ...
  const { data, error } = await supabase
    ...
    .select('id,sender_user_id,receiver_user_id,status,message,expires_at,responded_at,created_at')
    .single()

  if (error || !data) {
    if (error?.code === '40001' || error?.message?.includes('friend_pair_retryable')) {
      return NextResponse.json({ error: 'friend_pair_retryable' }, { status: 409 })
    }
