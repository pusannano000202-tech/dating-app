...
import { BoundedMultipartError, readBoundedMultipartFormData } from '@/lib/http/bounded-multipart'
...
const MAX_PHOTO_MULTIPART_BYTES = MAX_PHOTO_COUNT * MAX_PHOTO_BYTES + MULTIPART_OVERHEAD_BYTES
...
export async function POST(request: NextRequest) {
  ...
  const completed = await refreshProfileCompletion(admin, userId)
  if (!completed.ok) return jsonError(completed.error, completed.status)
  if (!completed.isComplete) return jsonError('profile_onboarding_incomplete', 409)
...
export async function DELETE(request: NextRequest) {
  ...
  if (deleteRequest.all) {
    ...
    const refreshed = await refreshProfileCompletion(admin, userId)
    if (!refreshed.ok) {
      const rollback = storedPhotos.photos.length > 0
        ? await admin.from('photos').insert(storedPhotos.photos.map((photo) => ({ user_id: userId, ...photo })))
        : { error: null }
      return jsonError(rollback.error ? 'photo_delete_rollback_failed' : refreshed.error, refreshed.status)
    }
  ...
  const refreshed = await refreshProfileCompletion(admin, userId)
  if (!refreshed.ok) {
    const { error: rollbackError } = await admin.from('photos').insert({
      user_id: userId,
      storage_path: storagePath,
      sort_order: ownedPhoto.sort_order,
    })
    return jsonError(rollbackError ? 'photo_delete_rollback_failed' : refreshed.error, refreshed.status)
  }
...
> {
  ...
  try {
    formData = await readBoundedMultipartFormData(request, MAX_PHOTO_MULTIPART_BYTES)
  } catch (error) {
    if (error instanceof BoundedMultipartError && error.code === 'payload_too_large') {
...
async function refreshProfileCompletion(
  ...
  userId: string,
): Promise<{ ok: true; isComplete: boolean } | { ok: false; error: string; status: number }> {
  const { data, error } = await admin.rpc('refresh_profile_onboarding_completion', {
    p_user_id: userId,
  })
  if (error) return { ok: false, error: 'profile_update_failed', status: 503 }
  return { ok: true, isComplete: data === true }
}

function guardMutationOrigin(request: NextRequest): NextResponse | null {
  try {
    assertTrustedMutationOrigin(request)
    return null
  } catch (error) {
    const status = error instanceof TrustedOriginError ? error.status : 403
    return jsonError(status === 503 ? 'photo_service_unavailable' : 'request_not_allowed', status)
  }
}
