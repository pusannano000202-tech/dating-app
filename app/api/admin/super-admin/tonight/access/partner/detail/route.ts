...
export async function GET(request: Request) {
  try {
    ...
    const { data, error } = await supabase.rpc('super_admin_get_venue_partner_membership', {
    ...
    const hydrated = await hydrateTonightAccessMemberships(
      service,
      [membership] as Array<Record<string, unknown> & { user_id: string; venue_id: string }>,
      { includeVenueName: true },
    )
    if (hydrated.error || !hydrated.data?.[0]) {
      return privateJson({ error: 'service_unavailable' }, 503)
    }
    return privateJson({ membership: hydrated.data.slice(0, 1)[0] })
  } catch (error) {
