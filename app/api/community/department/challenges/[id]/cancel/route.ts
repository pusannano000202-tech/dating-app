...
export async function POST(request: Request, context: Context) {
  try {
    ...
    const body = await readStrictJson(request, ['reason', 'expected_revision', 'idempotency_key'])
    const supabase = createSupabaseRequestClient(request)
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) return meetupJson({ error: 'Unauthorized' }, 401)
    const { data, error } = await supabase.rpc('cancel_my_department_challenge', {
      p_challenge_id: asUuid(id, 'challenge_id'),
      p_reason: asRequiredString(body.reason, 'reason', { minLength: 2, maxLength: 240 }),
