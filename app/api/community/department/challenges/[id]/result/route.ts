...
export async function POST(request: Request, context: Context) {
  try {
    ...
    const body = await readStrictJson(request, ['own_score', 'opponent_score', 'expected_revision', 'idempotency_key'])
    const supabase = createSupabaseRequestClient(request)
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) return meetupJson({ error: 'Unauthorized' }, 401)
    const { data, error } = await supabase.rpc('confirm_my_department_challenge_result', {
      p_challenge_id: asUuid(id, 'challenge_id'),
      p_own_score: asInteger(body.own_score, 'own_score', { min: 0, max: 999 }),
      p_opponent_score: asInteger(body.opponent_score, 'opponent_score', { min: 0, max: 999 }),
