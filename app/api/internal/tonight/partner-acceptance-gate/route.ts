...
export async function GET(request: Request) {
  ...
  const policy = getTonightDueRoundPolicy('partner_acceptance')
  ...
  for (const round of rounds) {
    const expired = await service.rpc('service_expire_tonight_partner_acceptance_gate', {
      p_round_id: round.id,
      p_idempotency_key: `tonight-partner-gate-${MARKET_CODE}-${round.serviceDate}`,
    })
