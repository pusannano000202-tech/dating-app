    ...
    <main className="min-h-screen bg-[#FFF8F5] pb-28 text-boot-ink">
      ...
      <div className="mx-auto w-full max-w-4xl px-4 py-6 sm:px-6 sm:py-9">
        <section className="relative min-h-[280px] overflow-hidden rounded-lg text-white sm:min-h-[360px]">
          <Image src="/images/match/couple-play-20260915.webp" alt="보드게임 카페에서 함께 웃는 두 커플의 분위기 예시" fill priority sizes="(max-width: 768px) 100vw, 900px" className="object-cover" />
          ...
          <div className="absolute inset-x-0 bottom-0 p-5 sm:p-7">
            <p className="text-[11px] font-black text-[#FFC7B5]">날짜를 골라 만나는 커플 2팀</p>
        ...
        <Link href="/match/calendar?audience=couple" className="mt-5 flex min-h-14 items-center justify-center rounded-2xl bg-[#B64A3B] px-4 font-bold text-white">커플 이벤트 날짜 고르기</Link>
        <details className="mt-5 rounded-2xl border border-[#E8CFC7] bg-white p-4"><summary className="cursor-pointer font-bold">기존 커플 신청 확인</summary><div className="mt-4"><QuantumCoupleDoubleDate devPreview={devPreview} /></div></details>
      </div>
    </main>
  )
}
