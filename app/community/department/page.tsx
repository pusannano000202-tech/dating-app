import {readLeagueLocation} from '@/lib/meetups/league-navigation'
...
export default async function DepartmentCommunityPage({ searchParams }: {
  searchParams: Promise<{ category?: string | string[]; sport?: string | string[]; legacy?: string | string[]; invite?: string | string[]; challenge?: string | string[]; team?: string | string[]; opponent?: string | string[]; opponent_cursor?: string | string[]; panel?: string | string[]; slot?: string | string[]; view?: string | string[]; source?: string | string[] }>
}) {
  if (!isCommunityFeatureEnabled()) return <CommunityComingSoon kind="meetups" />
  const params = await searchParams
  const { category, sport, invite, challenge, team, panel, slot } = params
  const location = readLeagueLocation(new URLSearchParams(Object.entries(params).filter((entry): entry is [string, string] => typeof entry[1] === 'string')))
  ...
  return <DepartmentLeagueJourney initialLocation={location} initialSport={selectedSport} initialInviteId={uuid(invite) ? invite : undefined} initialChallengeId={uuid(challenge) ? challenge : undefined} initialTeamId={uuid(team) ? team : undefined} initialReview={panel === 'applications' && uuid(challenge) && uuid(team) && !!selectedSport} initialReviewSlot={reviewSlot} initialResult={panel === 'result' && uuid(challenge)} />
}
