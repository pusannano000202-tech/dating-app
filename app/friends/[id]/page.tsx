...
export default function FriendDatePage() {
  ...
  const [friendScene, setFriendScene] = useState<FriendSceneSummary | null>(null)
  const [friendSceneReady, setFriendSceneReady] = useState(preview)

  const friendName = useMemo(
    () => friendProfile?.display_name || proposals[0]?.other_display_name || 'Quantum 친구',
    [friendProfile?.display_name, proposals],
  )
  ...
  const refreshFriendScene = useCallback(async () => {
    if (preview) {
      setFriendScene(null)
      setFriendSceneReady(true)
      return
    }
    try {
      ...
      setFriendScene(parsed.find((row) => row.friendUserId === friendUserId) ?? null)
      setFriendSceneReady(true)
    } catch {
      setFriendScene(null)
      setFriendSceneReady(false)
    }
  }, [friendUserId, preview])

  useEffect(() => {
    void refresh()
    void refreshFriendScene()
  }, [refresh, refreshFriendScene])
    ...
    <main className="min-h-screen booting-paper px-4 pb-28 pt-6 text-boot-ink">
      <div className="mx-auto w-full max-w-md">
          ...
          <section className="mt-5 overflow-hidden rounded-lg border border-boot-hairline bg-white shadow-sm">
            <div className="grid grid-cols-[96px_1fr] gap-4 p-4">
              ...
              <div className="min-w-0 self-center">
                ...
                <div className="mt-2"><FriendSceneBadge kind={friendSceneReady ? (friendScene?.kind ?? 'unclassified') : null} evidence={friendSceneReady ? (friendScene?.evidence ?? 'unknown') : null} /></div>
              </div>
            </div>
          </section>
        ) : null}

        <Link href={`/friends/${encodeURIComponent(friendUserId)}/chat`} className="mt-5 flex min-h-12 items-center justify-center gap-2 rounded-xl bg-boot-primary px-4 py-3 text-sm font-black text-white">
          <MessageCircle size={18} />친구 1:1 채팅 열기
        </Link>
        ...
        <section className="mt-5 flex items-start gap-3 rounded-lg border border-emerald-200 bg-emerald-50 p-4">
          ...
          <p className="text-sm leading-6 text-emerald-950">약속은 상대가 제안을 수락해야 확정돼요. 시간과 장소는 위의 친구 채팅에서 먼저 이야기할 수 있어요.</p>
        </section>
