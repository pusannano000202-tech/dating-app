...
export default function FriendChatRoom({ friendUserId }: { friendUserId: string }) {
  ...
  return <div className="flex min-h-[calc(100dvh-10rem)] flex-col">
    ...
    <section ref={scroller} aria-label={`${friendName}님과의 메시지`} className="mt-4 max-h-[55dvh] min-h-56 flex-1 overflow-y-auto rounded-2xl border border-boot-hairline bg-white p-3">
      ...
      {access === 'revoked' ? <p className="p-8 text-center text-sm text-boot-muted">현재 이 대화를 열 수 없어요.</p> : loading ? <div className="flex min-h-48 items-center justify-center"><Loader2 aria-label="대화 불러오는 중" className="animate-spin text-boot-primary" /></div> : error || access !== 'active' ? <p className="p-8 text-center text-sm leading-6 text-boot-muted">대화 정보를 확인할 수 없어요.<br />다시 불러오기를 눌러 주세요.</p> : messages.length === 0 ? <div className="p-8 text-center"><MessageCircle className="mx-auto text-boot-primary" /><p className="mt-3 text-sm font-black">첫 메시지를 보내 보세요</p><p className="mt-2 text-xs leading-5 text-boot-muted">전화번호를 공개하지 않고<br />시간과 장소를 정할 수 있어요.</p></div> : <div className="space-y-3">{messages.map((row, index) => { const day = new Date(row.createdAt).toLocaleDateString('ko-KR'); const previousDay = index ? new Date(messages[index-1].createdAt).toLocaleDateString('ko-KR') : ''; return <div key={row.id}>{day !== previousDay ? <p className="my-3 text-center text-[11px] font-bold text-boot-muted">{day}</p> : null}<article className={`flex ${row.isMine ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[85%] rounded-2xl px-4 py-3 ${row.isMine ? 'rounded-br-md bg-boot-primary text-white' : 'rounded-bl-md bg-boot-paper text-boot-ink'}`}><p className="whitespace-pre-wrap break-words text-sm leading-6 [overflow-wrap:anywhere]">{row.body}</p><div className="mt-1 flex justify-end gap-1 text-[10px] opacity-70"><time dateTime={row.createdAt}>{new Date(row.createdAt).toLocaleTimeString('ko-KR',{hour:'2-digit',minute:'2-digit'})}</time>{row.isMine && peerReadIndex >= index ? <span>읽음</span> : null}</div></div></article></div>})}</div>}
    </section>
    {access === 'active' && <form onSubmit={send} className="mt-3 rounded-2xl border border-boot-hairline bg-white p-3 shadow-sm"><label htmlFor="friend-chat-message" className="sr-only">친구에게 보낼 메시지</label><div className="flex items-end gap-2"><textarea id="friend-chat-message" value={draft} onChange={(event) => setDraft(event.target.value)} maxLength={1000} rows={2} disabled={sending} placeholder="메시지를 입력하세요" className="min-h-12 min-w-0 flex-1 resize-y rounded-xl border border-boot-hairline bg-boot-paper px-3 py-2 text-sm outline-none focus:border-boot-primary" /><button type="submit" disabled={sending || !draft.trim() || [...draft].length > 1000} aria-label="메시지 보내기" className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-boot-primary text-white disabled:opacity-40">{sending ? <Loader2 size={18} className="animate-spin" /> : <Send size={18} />}</button></div><p className="mt-1 text-right text-xs text-boot-muted">{[...draft].length}/1000</p></form>}
  </div>
}
