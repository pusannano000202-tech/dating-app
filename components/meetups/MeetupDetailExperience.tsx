...
export default function MeetupDetailExperience({ meetupId }: { meetupId: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const response = await fetch(`/api/meetups/${encodeURIComponent(meetupId)}`, { cache: 'no-store' })
      const payload = await response.json().catch(() => null) as { meetup?: MeetupDetail; error?: string } | null
      if (!response.ok || !payload?.meetup) throw new Error(payload?.error ?? 'load_failed')
      ...
      const [chatResponse, guideResponse] = await Promise.all([
        fetch(`/api/meetups/${encodeURIComponent(meetupId)}/chat`, { cache: 'no-store' }),
        fetch(`/api/meetups/${encodeURIComponent(meetupId)}/guide`, { cache: 'no-store' }),
      ])
  ...
  const place = useMemo(() => meetup ? projectLegacyMeetupPlace({ meetupId: meetup.id, placeName: meetup.place_name, category: meetup.category }) : null, [meetup])
  ...
  async function toggleMembership() {
    if (!meetup) return
    const leaving = meetup.joined
    if (leaving && !window.confirm('개인 나가기를 하면 새 채팅을 보낼 수 없어요. 모임 전체는 취소되지 않습니다.')) return
    await request(`/api/meetups/${encodeURIComponent(meetup.id)}/join`, { method: leaving ? 'DELETE' : 'POST' })
  }
  ...
  async function updateSchedule() {
    ...
    await request(`/api/meetups/${encodeURIComponent(meetup.id)}/schedule`, jsonPost({
      ...
      expected_revision: meetup.revision,
      ...
      <div className="mx-auto w-full max-w-3xl space-y-5">
        ...
        <section className="rounded-[12px] border border-boot-hairline bg-white p-5 shadow-sm">
          ...
          <dl className="mt-4 grid gap-2 text-sm font-bold text-boot-muted sm:grid-cols-3">
            ...
            <div id="meetup-place" className="flex gap-2"><MapPin size={17} /><span>{meetup.place_name}</span></div>
          ...
          {active && !meetup.is_host ? <button type="button" disabled={busy || (!meetup.joined && meetup.status === 'full')} onClick={() => void toggleMembership()} className="mt-4 min-h-12 w-full rounded-[8px] bg-boot-primary px-4 text-sm font-black text-white disabled:opacity-45">{meetup.joined ? '개인 나가기' : '이 모임에 참여하기'}</button> : null}
        </section>

        {meetup.is_host && active ? <section className="rounded-[12px] border border-boot-hairline bg-white p-5"><h2 className="font-black">주최자 일정 관리</h2><p className="mt-1 text-xs font-bold text-boot-muted">일정 변경은 참가자 상세의 변경 기록에 남고 최신 revision으로만 저장돼요.</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><input aria-label="새 시작 시간" type="datetime-local" value={scheduleStart} onChange={(event) => setScheduleStart(event.target.value)} className={inputClass} /><input aria-label="새 종료 시간" type="datetime-local" value={scheduleEnd} onChange={(event) => setScheduleEnd(event.target.value)} className={inputClass} /><input aria-label="새 장소" value={schedulePlace} onChange={(event) => setSchedulePlace(event.target.value)} className={`${inputClass} sm:col-span-2`} /></div><div className="mt-3 grid gap-2 sm:grid-cols-3"><button type="button" disabled={busy} onClick={() => void updateSchedule()} className={primaryButton}>일정 변경 저장</button><button type="button" disabled={busy} onClick={() => void completeMeetup()} className={secondaryButton}>모임 전체 종료</button><button type="button" disabled={busy} onClick={() => void cancelMeetup()} className="min-h-11 rounded-[8px] border border-boot-coral/30 bg-white px-3 text-sm font-black text-boot-coral">모임 전체 취소</button></div></section> : null}

        {meetup.joined && guide ? <LiveActivityGuide guide={guide} busy={busy} onAction={(action, sceneId) => { void guideAction(action, sceneId) }} /> : null}

        {meetup.joined ? <section id="meetup-chat" className="scroll-mt-4 rounded-[12px] border border-boot-hairline bg-white p-5"><div className="flex items-center gap-2"><MessageCircle size={18} className="text-boot-primary" /><h2 className="font-black">참가자 대화</h2></div><p className="mt-1 text-xs font-bold leading-5 text-boot-muted">참가 중에는 별칭으로 대화하고, 나가기·취소·종료 뒤에는 새 메시지를 보낼 수 없어요. 연락처 공유는 차단돼요.</p><div className="mt-4 max-h-72 space-y-2 overflow-y-auto">{chat?.messages.length ? chat.messages.map((item) => <p key={item.id} className="rounded-[8px] bg-boot-soft px-3 py-2 text-sm"><b className="mr-2 text-boot-primary">{item.sender_alias}</b>{item.message}</p>) : <p className="text-sm font-bold text-boot-muted">아직 메시지가 없어요.</p>}</div>{chat?.phase === 'send' ? <div className="mt-4 flex gap-2"><input aria-label="참가자 대화 메시지" maxLength={1000} value={message} onChange={(event) => setMessage(event.target.value)} className={`${inputClass} min-w-0 flex-1`} /><button type="button" disabled={busy || !message.trim()} onClick={() => void sendMessage()} className={`${primaryButton} px-4`}>전송</button></div> : <p className="mt-3 text-xs font-black text-boot-muted">현재 대화는 읽기 전용이에요.</p>}</section> : null}
