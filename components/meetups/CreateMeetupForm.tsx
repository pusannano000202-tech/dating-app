...
export default function CreateMeetupForm() {
  ...
  const [genderMode, setGenderMode] = useState<MeetupGenderMode>('all')
  const [scopeType, setScopeType] = useState<MeetupScope>('school')
      ...
      <div className="mx-auto w-full max-w-2xl">
        <header className="flex items-start gap-3 border-b border-boot-hairline pb-4">
          ...
          <div>
            ...
            <p className="mt-2 text-sm font-bold leading-6 text-boot-muted">같이 할 활동과 약속, 참여 성별 조건을 정해요.</p>
          </div>
        </header>
        ...
        <form onSubmit={submit} className="mt-5 space-y-6">
          ...
          <fieldset>
            <legend className="text-sm font-black">참여 성별 조건</legend>
            <p className="mt-1 text-xs font-bold leading-5 text-boot-muted">등록 프로필 성별로 개설·참가 조건을 확인해요. 실명 인증 기능은 아니에요.</p>
            <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-3">
              {MEETUP_GENDER_MODES.map((mode) => {
                const selected = genderMode === mode
                return (
                  <button
                    key={mode}
                    type="button"
                    ...
                    className={`min-h-11 rounded-[8px] border px-3 text-sm font-black ${selected ? 'border-boot-primary bg-boot-primary text-white' : 'border-boot-hairline bg-white text-boot-muted'}`}
                  >
          ...
          <fieldset>
            <legend className="text-sm font-black">모집 범위</legend>
            <p className="mt-1 text-xs font-bold leading-5 text-boot-muted">학과는 가입 때 직접 입력한 정보를 서버에서 정규화해 확인해요. 공식 재학 인증 표시는 아니에요.</p>
            <div className="mt-3 grid grid-cols-2 gap-2">
              {([['school', '학교 전체'], ['department', '내 학과']] as const).map(([scope, label]) => (
                <button
                  key={scope}
                  type="button"
                  aria-pressed={scopeType === scope}
                  onClick={() => setScopeType(scope)}
                  className={`min-h-11 rounded-[8px] border px-3 text-sm font-black ${scopeType === scope ? 'border-boot-primary bg-boot-primary text-white' : 'border-boot-hairline bg-white text-boot-muted'}`}
                >
                  {label}
                </button>
              ))}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <label className="block">
              ...
              <input
                ...
                onChange={(event) => {
                  ...
                  if (!endsAt && value) setEndsAt(toLocalDateTime(new Date(new Date(value).getTime() + 2 * 60 * 60 * 1000)))
                }}
                min={minimumSchedule}
                required
                className="mt-2 min-h-12 w-full rounded-[8px] border border-boot-hairline bg-white px-4 text-sm font-bold outline-none focus:border-boot-primary focus:ring-2 focus:ring-boot-primary/20"
            ...
            <label className="block">
              <span className="flex items-center gap-2 text-sm font-black"><CalendarDays size={16} /> 끝나는 시간</span>
              <input
                type="datetime-local"
                value={endsAt}
                onChange={(event) => setEndsAt(event.target.value)}
                min={scheduledAt || minimumSchedule}
                required
                className="mt-2 min-h-12 w-full rounded-[8px] border border-boot-hairline bg-white px-4 text-sm font-bold outline-none focus:border-boot-primary focus:ring-2 focus:ring-boot-primary/20"
              />
            </label>
