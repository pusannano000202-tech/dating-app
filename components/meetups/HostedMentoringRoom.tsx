...
export default function HostedMentoringRoom({id,chatOnly=false,readOnly=false}:{id:string;chatOnly?:boolean;readOnly?:boolean}){
 ...
 const chat=useRef<HTMLDivElement|null>(null),pending=useRef<{signature:string;id:string}|null>(null)
 useEffect(()=>{setDraft('');setPanel(null);setTarget('');setReason('');setNotice('');setAt('');setPlace('');pending.current=null},[resource.account,id])
 useSocialChatRead('mentoring',id,room?.messages??[],{root:chat,enabled:!!room?.joined&&chatOnly&&!resource.error})
 const active=!!room?.joined&&room.status!=='closed'&&!readOnly&&!resource.busy
 ...
 async function send(){
  ...
  const signature=JSON.stringify([resource.account,id,draft.trim()])
  if(pending.current?.signature!==signature)pending.current={signature,id:crypto.randomUUID()}
  if(await act('message',{text:draft.trim(),client_id:pending.current.id})){setDraft('');pending.current=null}
 }
 const Container=chatOnly?'section':'main'
 return <Container className={chatOnly?s.section:s.page} key={`${resource.account}:${id}`}>
  ...
  {room.joined||room.my_role!==null?<>
   ...
   {chatOnly?<><div className={s.chat} ref={chat} role="log" aria-label="멘토링 대화" aria-live="polite">{room.messages.length?room.messages.map(m=><article className={`${s.message} ${m.mine?s.mine:''}`} key={m.id} data-social-message-id={m.id}><small>{m.alias}</small><p>{m.text}</p></article>):<p className={s.empty}>한 가지 질문이나 가볍게 나누고 싶은 경험으로 인사해 보세요.</p>}</div><form className={s.composer} onSubmit={e=>{e.preventDefault();void send()}}><textarea rows={1} aria-label="멘토링 메시지" placeholder="편하게 인사해 보세요" value={draft} maxLength={1000} disabled={!active} onChange={e=>setDraft(e.target.value)}/><button className={s.button} disabled={!active||!draft.trim()} aria-label="메시지 보내기"><Send size={18}/></button></form></>:<Link href={applicationChatPath(id,'mentoring')} className={`${s.button} w-full`}>모임 채팅으로<ArrowRight size={17}/></Link>}
