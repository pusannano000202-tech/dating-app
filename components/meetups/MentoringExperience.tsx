...
function MentoringContent({demo=false,chatOnly=false,fixedSessionId,readOnly=false}:MentoringExperienceProps){
 ...
 const content=<Container className={chatOnly?undefined:s.page}><div className={s.shell}>
 ...
 return chatOnly?<SocialMessenger scope={snapshot?.session_id??fixedSessionId??'mentoring'} root={chatReadRoot}
  messages={!targetMismatch&&!problem&&phase==='active'?snapshot?.messages.map(message=>({id:message.id,alias:message.alias,text:message.text,createdAt:message.created_at,isMe:message.mine}))??[]:[]}
  loading={loading} error={problem?t(`mentor.${problem}`):undefined} onRetry={()=>void request()} readOnly={readOnly||!!snapshot&&(phase!=='active'||remaining<=0)}
  management={content} empty={phase==='active'?t('mentor.emptyChat'):'참여 상태는 방 메뉴에서 확인할 수 있어요'}
  composer={<SocialChatComposer value={draft} onChange={setDraft} onSend={()=>void send()} busy={busy} disabled={!snapshot||targetMismatch||loading||!!problem||phase!=='active'||remaining<=0||readOnly} label={t('mentor.message')} placeholder={t('mentor.message')}/>}/>:content
}
