...
function NativeApplicationForm({kind,id}:{kind:NativeAdmissionKind;id:string}){
 ...
 return <MeetupApplicationFlow key={`${resource.account}:${kind}:${id}:${metadata.role??''}`} room={context.room} accountKey={resource.account??null} quote={context.quote} policy={context.policy} meetup={{title:context.roomDetails.title,activityLabel:kind==='study'?'우리 과 전공 스터디':metadata.role==='mentor'?'멘토링 · 경험을 나눌래요':'멘토링 · 도움을 구할래요',imageSrc:photo.src,imageAlt:photo.description,summary:`${context.roomDetails.memberCount} / ${context.roomDetails.capacity}명 · 보증금 확인 후 신청, 방장 수락 후 채팅 입장`,scheduleLabel:'날짜는 채팅에서 함께 정해요',locationLabel:'장소는 채팅에서 함께 정해요'}} onCancel={()=>router.push(back)}/>
 // No onSubmit until trusted provider verification is connected. Preparation is not paid admission.
}
