...
export default function HostedMentoringLobby(){
 ...
 {!creating?<section className={s.section} aria-label="멘토링 합류 대기판"><div className={s.bar}><div><h2>내 이야기를 나눌 사람, 기다려요</h2><p>원하는 역할로 등록하고 함께하자는 초대를 받아보세요.</p></div></div><Link className={s.secondary} href={`/meetups/candidates?kind=mentoring&key=${topic}`}>멘토링 합류 대기판 →</Link>{process.env.NODE_ENV==='development'?<Link className={s.back} href={`/meetups/dev-candidates?kind=mentoring&key=${topic}`}>예시로 흐름 둘러보기 · 실제 등록 아님</Link>:null}</section>:null}
