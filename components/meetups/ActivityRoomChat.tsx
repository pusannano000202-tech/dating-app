...
export default function ActivityRoomChat({ roomId }: { roomId: string }) {
  ...
  async function send() {
    ...
    if (!room || !text || mutation.current || text.length > 1000 || error) return
  ...
  return <main className={styles.page}><div className={styles.shell}>
    <Link href={lobbyHref} className={styles.back}><ArrowLeft size={18} />모임방 목록</Link>
    ...
    {room ? <>
      <header className={styles.chatHeader}><p className={styles.eyebrow}>{MEETUP_GENDER_LABELS[room.gender_mode]} · {room.room_number}번 방</p><h1>{getActivityRoomDefinition(room.activity_key)?.title}</h1><p>{room.member_count}/{room.capacity}명 · {room.members.map(member => `${member.alias}${member.is_me ? ' (나)' : ''}`).join(' · ')}</p></header>
      <aside className={styles.hint}><MessageCircle size={20} /><div><strong>먼저 이전 대화에서 약속을 확인해요</strong><p>새로 들어와도 입장 전 대화를 볼 수 있어요. 이미 정한 시간·장소를 읽고 인사해 주세요. 아직 정하지 않았다면 함께 조율해요.</p></div></aside>
      ...
      <div className={styles.messages} ref={messagesRef} role="log" aria-label="모임방 대화" aria-live="polite" tabIndex={0} onScroll={() => { const list = messagesRef.current; if (list) nearBottom.current = list.scrollHeight - list.scrollTop - list.clientHeight < 80 }}>
        {history.messages.length === 0 ? <div className={styles.status}>첫 인사를 건네 볼까요?<br />“안녕하세요! 몇 시가 편하세요?”</div> : history.messages.map(item => <article className={`${styles.message} ${item.is_me ? styles.messageMine : ''}`} key={item.id}><p className={styles.sender}>{item.is_me ? '나' : item.sender_alias}</p><p className={styles.bubble}>{item.message}</p><time className={styles.time} dateTime={item.created_at}>{new Date(item.created_at).toLocaleString('ko-KR', { month: 'numeric', day: 'numeric', hour: 'numeric', minute: '2-digit' })}</time></article>)}
      </div>
      <form className={styles.composer} onSubmit={event => { event.preventDefault(); void send() }}><textarea aria-label="메시지 입력" placeholder="같이할 친구들에게 한마디" rows={1} value={message} maxLength={1000} disabled={busy} onChange={event => { setMessage(event.target.value); if (pendingSend.current?.message !== event.target.value.trim()) pendingSend.current = null }} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); void send() } }} /><button type="submit" aria-label="메시지 보내기" disabled={busy || !message.trim() || !!error}><Send size={20} /></button></form>
