...
export default function MeetupIdeaCylinder({ ideas, genderMode = 'all' }: MeetupIdeaCylinderProps) {
  ...
  return (
    <div
      ...
      className="relative mt-4 overflow-clip bg-[#121821] px-3 pb-5 pt-4 text-white sm:px-7 sm:pb-7"
    >
      ...
      >
        {ideas.map((idea, index) => {
          ...
          return (
            ...
            >
              <span data-part="idea-photo" className="absolute inset-x-0 top-0 block h-[46%] bg-[#25303C]">
                <Image
                  src={idea.imageSrc}
                  alt={idea.imageAlt}
                  fill
              ...
              <span className="absolute inset-x-0 bottom-0 top-[46%] bg-[#1B232E] p-4">
                <span className="text-[11px] font-black text-[#F3B95F]">{idea.label}</span>
                <span className="mt-1 block text-xl font-black leading-tight sm:text-2xl">{idea.title}</span>
                <span className="mt-2 block text-[13px] font-bold leading-5 text-white/78 sm:text-sm sm:leading-6">{idea.description}</span>
                <span className="mt-3 flex items-center gap-1 text-xs font-black text-white/85">
                  <UsersRound size={15} /> 방마다 {getMeetupCapacityRecommendation(idea.category)}명
                </span>
              </span>
              {active ? (
      ...
      <Link
        href={`/meetups/activities/${activeIdea.id}/rooms?gender_mode=${genderMode}`}
      ...
      >
        {activeIdea.title} 모임방 보기 <ArrowRight size={18} />
      </Link>
      <p className="mt-3 text-center text-xs font-bold leading-5 text-white/70">방은 자동으로 준비돼요 · 정원 도달 시 다음 방 · 참여 후 채팅 약속</p>
    </div>
  )
}
