...
export default function LeagueLobby({sport,state,own,revision,demo,onDemo,onRoster,onPaired,onRefresh,renderRoster,captainOnly=false,gap=200,opponentId,opponentCursor,onOpponentChange}:{sport:LeagueSport;captainOnly?:boolean;gap?:200|300;state:JourneyState;own:JourneyTeam|null;revision:number;demo:boolean;onDemo:(action:LeagueDemoAction)=>void;onRoster:()=>void;onPaired:(id?:string)=>void;onRefresh:()=>Promise<unknown>;renderRoster:(players:JourneyPlayer[],selected:string,onSelect:(slot:string)=>void)=>ReactNode;opponentId?:string|null;opponentCursor?:string|null;onOpponentChange?:(id:string|null,cursor:string|null)=>void}){
 ...
 const [live,setLive]=useState<LeagueLobbyState|null>(null),[loadedKey,setLoadedKey]=useState(''),[error,setError]=useState(''),[busy,setBusy]=useState(false),[localOpponent,setLocalOpponent]=useState<{id:string|null;cursor:string|null}>({id:null,cursor:null}),[view,setView]=useState<'detail'|'proposal'|'waiting'>('detail'),[side,setSide]=useState<'own'|'other'>('other'),[slot,setSlot]=useState(''),[recipient,setRecipient]=useState(''),[notice,setNotice]=useState('')
 const selected=opponentId===undefined?localOpponent.id:opponentId,selectedCursor=selected?(opponentId===undefined?localOpponent.cursor:opponentCursor??null):null
 const generation=useRef(0),mutation=useRef(false),pageCursors=useRef(new Map<string,string|null>())
 ...
 const requestKey=`${sport}:${acceptedTeamId??''}:${selected??''}:${selectedCursor??''}`
 const snapshot=demo?demoSnapshot(state,own):loadedKey===requestKey?live:null
 const candidate=snapshot?.teams.find(team=>team.team_id===selected&&!team.is_mine)
 const candidates=snapshot?.teams.filter(team=>!team.is_mine)??[]
 const load=useCallback(async(cursor=selectedCursor??undefined,append=false)=>{
  if(demo)return
  ...
  try{
   ...
   if(version===generation.current){if(!append)pageCursors.current.clear();for(const team of parsed.teams)pageCursors.current.set(team.team_id,cursor??null);setLive(previous=>append&&previous?{...parsed,teams:[...previous.teams.filter(old=>!parsed.teams.some(team=>old.team_id===team.team_id)),...parsed.teams]}:parsed);setLoadedKey(requestKey);setError('')}
  }catch{if(version===generation.current){setLive(null);setError('대기 팀을 불러오지 못했어요. 다시 연결해 주세요.')}}
 },[demo,acceptedTeamId,sport,requestKey,selectedCursor])
 useEffect(()=>{if(demo)return;setLive(null);setError('');void load();const timer=window.setInterval(()=>{if(!mutation.current&&document.visibilityState==='visible'){void load();if(!captainOnly&&viewerAccepted)void onRefresh()}},8000);return()=>{invalidateLoad();window.clearInterval(timer)}},[load,demo,captainOnly,viewerAccepted,onRefresh,invalidateLoad])
 useEffect(()=>{setSide('other');setSlot('');setView('detail');setNotice('')},[selected,sport,acceptedTeamId])
 ...
 const changeOpponent=(id:string|null,cursor:string|null)=>{setLocalOpponent({id,cursor});onOpponentChange?.(id,cursor);setView('detail');setNotice('')}
 const selectTeam=(team:LeagueLobbyTeam)=>{changeOpponent(team.team_id,pageCursors.current.get(team.team_id)??null);setSide('other');setSlot('');setView(team.outgoing?'waiting':'detail')}
 ...
 return <section className={s.lobby} aria-label="리그 대기실">
  ...
  {selected&&candidate?<>
   <button type="button" className={s.back} onClick={()=>changeOpponent(null,null)}>← 대기 팀 목록</button>
  ...
  </>:selected?<div className={s.card}><button type="button" className={s.back} onClick={()=>changeOpponent(null,null)}>← 대기 팀 목록</button>{!snapshot&&!error?<p className={s.notice} role="status">선택한 상대 팀의 현재 상태를 확인하고 있어요.</p>:snapshot?<><h2>이 상대 팀을 현재 대기 목록에서 찾지 못했어요</h2><p>팀이 대기를 마쳤거나 목록이 바뀌었을 수 있어요. 대기 팀 목록에서 다시 확인해 주세요.</p></>:<p>상대 팀 상태를 확인하지 못했어요. 다시 연결하거나 대기 팀 목록으로 돌아가 주세요.</p>}</div>:<>
   <header className={s.heading}><div><span className={s.eyebrow}>CAMPUS MATCH</span><h2>대결 상대 찾기</h2></div><button type="button" className={s.back} disabled={busy} onClick={()=>{void onRefresh();void load()}}>새로고침</button></header>
   <div className={s.proposalSummary} aria-label="경기 제안 현황"><span>조회된 상대 {snapshot?candidates.length:'—'}팀</span><strong>받은 제안 {snapshot?candidates.filter(team=>team.incoming).length:'—'} · 보낸 제안 {snapshot?candidates.filter(team=>team.outgoing).length:'—'}</strong></div>
   <div className={s.ownTeam}><div><small>우리 팀</small><strong>{ownName}</strong><span>{accepted.length}/{LEAGUE_SPORTS[sport].capacity}명 · 팀 합계 {own?leagueTeamSum(own).toLocaleString():'—'}</span></div><button type="button" className={s.secondary} onClick={onRoster}>{own?.is_mine?'내 팀·포지션':'팀 참가하기'} →</button></div>
   ...
   {snapshot?.next_cursor?<button type="button" className={s.secondary} onClick={()=>void load(snapshot.next_cursor!,true)}>대기 팀 더 보기</button>:null}
  </>}
 </section>
}
