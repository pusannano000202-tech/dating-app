...
import {LeagueTierPicker} from './LeagueTier'
...
export function useLeaguePositionInvites({sport,challengeId,demo,demoState,onDemo,onJourneyRefresh}:{sport:LeagueSport|null;challengeId:string|null;demo:boolean;demoState:LeagueInviteState|null;onDemo:(action:LeagueInviteDemoAction)=>void;onJourneyRefresh:()=>Promise<unknown>}){
 ...
 useEffect(()=>{if(demo)return;setLive(null);setLoading(!!sport);setBusy(false);mutation.current=false;pending.current=null;void load();const refreshVisible=()=>{if(sport&&!mutation.current&&document.visibilityState==='visible'){void load();void onJourneyRefresh()}};const timer=window.setInterval(refreshVisible,10000);window.addEventListener('focus',refreshVisible);document.addEventListener('visibilitychange',refreshVisible);return()=>{invalidate();window.clearInterval(timer);window.removeEventListener('focus',refreshVisible);document.removeEventListener('visibilitychange',refreshVisible)}},[load,demo,sport,challengeId,invalidate,onJourneyRefresh])
 async function command(action:'invite'|'accept'|'decline'|'cancel',args:Record<string,unknown>,demoAction:LeagueInviteDemoAction):Promise<MutationResult|null>{
  ...
  try{
   ...
   // Acceptance opens a separately authorized chat immediately. Do not make the
   // confirmed transition wait for refreshes of the invitation screen we leave.
   if(action!=='accept')await Promise.all([load(),onJourneyRefresh()])
...
export function LeagueInviteBell({controller,onOpen,demo=false}:{controller:LeagueInvitesController;onOpen:()=>void;demo?:boolean}){
 const count=controller.state?.incoming.filter(invite=>invite.status==='pending'&&Date.parse(invite.expires_at)>Date.now()).length
 return demo?<button type="button" className={s.bell} onClick={onOpen} aria-label={`예시 받은 초대 ${count??'확인 중'}`}><Bell size={16} aria-hidden="true"/><span>예시 초대</span><b>{count??'—'}</b></button>:<Link href="/notifications" className={s.bell} aria-label="알림함 열기"><Bell size={18} aria-hidden="true"/><span>알림함</span></Link>
}
...
export function LeagueInviteReceiver({sport,inviteId,controller,renderMap,onBack,onJoined}:{sport:LeagueSport;inviteId:string;controller:LeagueInvitesController;demo:boolean;renderMap:(players:JourneyPlayer[],slot:string,pending:{slot:string;alias:string}[])=>ReactNode;onBack:()=>void;onJoined:(challengeId:string,teamId:string)=>void}){
