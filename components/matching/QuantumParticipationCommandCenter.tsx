...
export default function QuantumParticipationCommandCenter({
  ...
  if (placement === 'home') {
    return <section aria-label="내 약속 진행 상태" data-lifecycle-stage={stage} className="rounded-xl border border-[#e8cfc4] bg-white px-4 py-3">
      <p className="text-[11px] font-bold text-[#a43f32]">{participation.event_mode === 'scheduled' ? '이번 주 만나기' : '오늘 밤 만나기'} · {copy.eyebrow}</p>
      <h2 className="mt-1 text-base font-black">{event.title}</h2>
      <p className="mt-1 text-xs leading-5 text-[#807169]">{lifecycle ? `${formatDateTime(lifecycle.starts_at)} · ${lifecycle.location_name || '장소 확정 대기'}` : '신청 완료 · 팀 배정 대기'}</p>
      {lifecycle ? <p className="mt-1 text-xs font-bold text-[#807169]">현재 {lifecycle.participant_counts.total}명 · 남 {lifecycle.participant_counts.male} · 여 {lifecycle.participant_counts.female}</p> : null}
      <Link href={actionHref} className="mt-2 flex min-h-11 items-center justify-between gap-2 text-sm font-bold text-[#a43f32]">{copy.action}<ArrowRight size={17} /></Link>
    </section>
  }
