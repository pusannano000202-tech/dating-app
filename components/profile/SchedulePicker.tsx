        ...
        return (
          <div
            ...
            className={`rounded-2xl transition-all duration-200 overflow-hidden ${
              slot.enabled ? 'bg-white/10' : 'bg-white/5'
            }`}
          >
            {/* 요일 토글 헤더 */}
            <button
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className={`text-sm font-medium ${slot.enabled ? 'text-white' : 'text-gray-500'}`}>
                  {slot.enabled ? `${slot.start} ~ ${slot.end}` : '선택 안 함'}
                </span>
              </div>
              <div
                className={`w-11 h-6 rounded-full transition-colors duration-200 relative ${
                  slot.enabled ? 'bg-purple-600' : 'bg-gray-700'
                }`}
              >
                <div
              ...
              <div className="px-4 pb-4 flex items-center gap-3">
                <div className="flex-1">
                  ...
                  <select
                    ...
                    className="w-full bg-white/10 text-white text-sm rounded-xl px-3 py-2.5 appearance-none border border-white/10 focus:outline-none focus:border-purple-500"
                  >
