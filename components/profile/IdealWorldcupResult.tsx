  ...
  return (
    <div className="flex flex-col items-center min-h-screen px-4 py-10">
      <div className="w-full max-w-md flex flex-col items-center">
        ...
        <div className="w-full rounded-3xl border border-boot-hairline bg-white p-5 shadow-sm mb-8">
          <p className="text-xs text-violet-600 font-bold tracking-widest uppercase mb-2">
            PRIMARY
          </p>
          ...
          {secondaryType && (
            <>
              <p className="mt-5 text-xs text-violet-600 font-bold tracking-widest uppercase">
                SECONDARY
              </p>
        ...
        {saveError && (
          <p className="text-sm text-rose-600 text-center mb-3">{saveError}</p>
        )}
        ...
        <button
          ...
          onClick={onRetry}
          disabled={saving}
          className="mt-3 w-full rounded-2xl border border-boot-hairline bg-white py-3 text-sm text-boot-muted transition-colors hover:text-boot-ink"
        >
          다시 하기
        </button>
      </div>
