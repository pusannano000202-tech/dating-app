...
test('my profile header exposes the compact profile and retry contract', async () => {
  ...
  assert.match(source, /profileState: 'loading' \| 'ready' \| 'error';/);
  assert.match(source, /displayName: string \| null;/);
  assert.match(source, /school: string \| null;/);
  assert.match(source, /primaryPhotoUrl: string \| null;/);
  assert.match(source, /progress: MyProfileProgress \| null;/);
  ...
  assert.match(source, /function initials\(displayName: string \| null\)/);
  ...
  assert.match(source, /resolvedProfileState === 'loading'/);
  assert.match(source, /프로필 정보를 불러오는 중/);
  assert.match(source, /resolvedProfileState === 'error'/);
  assert.match(source, /프로필 정보를 확인하지 못했어요/);
