...
const AUTH_CALLBACK_URL = 'http://localhost:3010/auth/callback'

const { workspaceRoot, runtimeRoot } = resolveCommunityVoiceLocalRoots(
  process.cwd(),
)
...
async function resolveSupabaseCliScript(dockerEnvironment) {
  ...
  for (const entry of entries
    ...
    try {
      ...
      if (
        metadata.version !== '2.116.0' ||
        metadata.bin?.supabase !== 'dist/supabase.js'
      )
        continue
...
function readLocalStatus(scriptPath, dockerEnvironment) {
  try {
    return JSON.parse(
      execFileSync(
        process.execPath,
        [scriptPath, 'status', '--workdir', runtimeRoot, '-o', 'json'],
        {
          cwd: workspaceRoot,
          encoding: 'utf8',
          timeout: 120_000,
    ...
    return JSON.parse(
      execFileSync(
        ...
        [
          ...
          'desktop-linux',
          'inspect',
          `supabase_auth_${COMMUNITY_VOICE_LOCAL.projectId}`,
          '--format',
          '{{json .Config.Env}}',
        ],
        {
...
function assertLocalDockerContext(dockerEnvironment) {
  try {
    ...
    if (
      ...
      error.message === 'unexpected_docker_context_endpoint'
    )
      throw error
...
async function assertPortAvailable() {
  await new Promise((resolve, reject) => {
    ...
    server.unref()
    server.once('error', () =>
      reject(new Error('social_scenes_port_3013_unavailable')),
    )
    server.listen(
      { host: '127.0.0.1', port: APP_PORT, exclusive: true },
...
try {
  ...
  const secrets = await readJson(
    secretsPath,
    'source_runtime_secrets_unavailable',
  )
  const localEnv = createIntegratedRuntimeEnvironment(status, secrets, smsTtl)
  const safeLocalEnv = buildCommunityVoiceChildEnvironment(
    process.env,
    localEnv,
  )
  const childEnv = {
    ...
    NEXT_DIST_DIR: '.next-social-scenes-local',
  ...
  const authPreflight = await preflightIntegratedLocalAuth({
    url: childEnv.NEXT_PUBLIC_SUPABASE_URL,
    publicKey:
      childEnv.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY ||
      childEnv.NEXT_PUBLIC_SUPABASE_ANON_KEY,
  })

  const sourceManifest = await readJson(
    manifestPath,
    'source_migration_manifest_unavailable',
  )
  const workspaceMigrationNames = (
    await readdir(join(workspaceRoot, 'supabase', 'migrations'), {
      withFileTypes: true,
    })
  )
    .filter((entry) => entry.isFile() && entry.name.endsWith('.sql'))
    .map((entry) => entry.name)
  const migrationBoundary = describeMigrationBoundary(
    workspaceMigrationNames,
    sourceManifest,
  )

  await access(join(workspaceRoot, 'node_modules', 'next', 'dist', 'bin', 'next'))
  console.log(
    JSON.stringify({
      application: APP_ORIGIN,
      localAuth: COMMUNITY_VOICE_LOCAL.apiUrl,
      authCallbackUrl: AUTH_CALLBACK_URL,
      authCallbackChanged: false,
      localhostCookiesMayBeSharedAcrossPorts: true,
      freshAuthOnPort3013Verified: false,
  ...
  console.warn(
    `Auth callback은 ${AUTH_CALLBACK_URL} 그대로입니다. ${APP_ORIGIN}에서는 기존 localhost 쿠키가 공유될 수 있지만 새 로그인 callback은 검증하지 않았습니다.`,
  )
  ...
  child.on('error', () => {
    console.error('social_scenes_local_ui_start_failed')
    process.exitCode = 1
  })
  child.on('exit', (code) => {
  ...
  console.error(
    error instanceof Error
      ? error.message
      : 'social_scenes_local_launcher_failed',
  )
  process.exitCode = 1
}
