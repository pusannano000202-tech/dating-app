...
import { integratedUiEnvironment, integratedUiLaunchInfo, resolveIntegratedUiPort } from './integrated-ui-config.mjs'
import { preflightIntegratedLocalAuth } from './integrated-auth-preflight.mjs'

const mode = process.argv[2]
const port = resolveIntegratedUiPort(process.argv.slice(3))
const env = integratedUiEnvironment(process.env, mode, port)
let authPreflight
...
if (process.exitCode !== 1) {
  ...
  const child = spawn(process.execPath, ['node_modules/next/dist/bin/next', 'dev', '-p', port, '-H', '127.0.0.1'], {
