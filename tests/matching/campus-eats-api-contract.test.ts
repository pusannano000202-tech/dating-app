...
test('Campus Eats URL selection never leaks a restaurant from the previous category', () => {
  const chickenIds = [
    'pnu:store:021:chicken',
    'pnu:store:022:chicken',
  ]

  assert.equal(
    normalizeCampusEatsSelectedRestaurantId(chickenIds, 'pnu:store:001:donkatsu'),
    'pnu:store:021:chicken',
  )
  assert.equal(
    normalizeCampusEatsSelectedRestaurantId(chickenIds, 'pnu:store:022:chicken'),
    'pnu:store:022:chicken',
  )
  assert.equal(normalizeCampusEatsSelectedRestaurantId([], 'pnu:store:001:donkatsu'), null)
})
