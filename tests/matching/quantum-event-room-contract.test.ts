...
test('confirmed event screens keep opponents private and expose consent-first continuation only after completion', () => {
  const application = readSource('components/matching/QuantumEventApplicationStatus.tsx')
  ...
  assert.match(application, /ScheduledContinuationStartButton/)
  assert.match(application, /친구는 자동으로 연결되지 않아요/)
  assert.doesNotMatch(application, /친구 프로필 보기/)
