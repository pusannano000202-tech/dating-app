...
test('atomically resolves an occurrence, saves readiness, and returns only the caller sealed role', () => {
  ...
  assert.match(route, /save_my_ready_quantum_event_meeting_moment_and_participate/i)
  assert.doesNotMatch(route, /\.rpc\('save_my_quantum_event_meeting_moment_and_participate'/i)
