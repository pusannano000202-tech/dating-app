...
test('web chat hub lists actual match rooms instead of a static redirect card', () => {
  ...
  // The combined chat hub now labels this section "내 대화방". Check the
  // actual confirmed/completed filter as well as the visible entry, not retired copy.
  assert.match(chatPage, /내 대화방/)
  assert.match(chatPage, /\['confirmed', 'completed'\]\.includes\(match\.match_status\)/)
})
