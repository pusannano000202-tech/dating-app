...
test('dev preview queue status persists from group create to the home task card', () => {
  ...
  assert.match(matchPage, /setDevPreviewSoloStatus\(devRequestedSoloStatus\)/)
...
test('solo introduction flow is separate from group creation', () => {
  ...
  assert.match(matchStartPage, /allowPreviewShortcuts/)
  ...
  assert.match(matchStartPage, /가매칭 미리보기/)
  assert.doesNotMatch(matchStartPage, /mock 가매칭 화면 보기/)
  assert.match(matchStartPage, /if \(steps\.every\(\(step\) => step\.done\) && mode === 'group'\) redirect\('\/group\/create'\)/)
})

test('solo queue preview stays dev-only while real product routes use API state', () => {
  const matchPage = readSource('app/match/page.tsx')
  ...
  assert.match(matchPage, /const devRequestedSoloStatus = params\.get\('sampleMatches'\) === '1'/)
  assert.match(matchPage, /setSoloQueueActive\(nextMode === 'solo' && devRequestedSoloStatus === 'in_pool'\)/)
  assert.match(matchPage, /setMatches\(devRequestedSoloStatus === 'matched'/)
  assert.match(matchPage, /setSoloQueueActive\(false\)/)
  assert.doesNotMatch(matchPage, /if \(explicitSoloPreview\)[\s\S]+DEV_SOLO_MATCHES/)
  ...
  assert.match(matchDetailPage, /const isDevMatchPreviewRoute = isDevMatchPreviewId\(matchId\)/)
  assert.match(matchDetailPage, /const canUseDevPreview = canUseDevMatchPreview\(matchId, isDevPreview\)/)
  assert.match(matchDetailPage, /if \(isDevMatchPreviewRoute && !canUseDevPreview\)/)
  assert.match(matchDetailPage, /if \(canUseDevPreview\)/)
  assert.doesNotMatch(matchDetailPage, /if \(isDevPreview\)[\s\S]{0,120}createDevMatchDetail/)
  assert.doesNotMatch(matchDetailPage, /matchId\.includes\('solo'\)/)
  assert.match(matchDetailPage, /return isDevSoloMatchPreviewId\(matchId\)/)
  ...
  assert.match(matchChatPage, /const canUseDevPreview = canUseDevMatchPreview\(matchId, isDevPreview\)/)
  assert.match(matchChatPage, /if \(isDevMatchPreviewRoute && !canUseDevPreview\)/)
  assert.match(matchChatPage, /if \(canUseDevPreview\)/)
  assert.doesNotMatch(matchChatPage, /if \(isDevPreview\)[\s\S]{0,120}DEV_CHAT_MESSAGES/)
