...
test('dev preview queue status persists from group create to the home task card', () => {
  ...
  assert.match(matchPage, /\{LEGACY_MATCH_ENTRY_VISIBLE && loading \? \(/)
})
...
test('match hub renders opponent cards only after match results exist', () => {
  ...
  assert.match(matchPage, /const shouldShowMatchingPool = LEGACY_MATCH_ENTRY_VISIBLE && !loading && !hasAnyStartedMatching && !hasMatchResults/)
  assert.match(matchPage, /LEGACY_MATCH_ENTRY_VISIBLE && shouldShowMatchingPool && devPreviewActive/)
  ...
  assert.match(matchPage, /<PostMatchFlowCard[\s\S]+match=\{shouldShowLegacyResult\}/)
  assert.match(matchPage, /\{LEGACY_MATCH_ENTRY_VISIBLE && shouldShowMatchingPool && devPreviewActive && \(/)
  assert.doesNotMatch(matchPage, /visibleMatches\.map/)
  assert.match(matchPage, /getOpponentFactChips\(shouldShowLegacyResult\)/)
  ...
  assert.match(matchPage, /!loading && !hasAnyStartedMatching && devPreviewActive && \(/)
...
test('home and match detail keep stale or terminal matching state non-actionable', () => {
  ...
  assert.doesNotMatch(matchDetail, /refreshConnections|setConnections|connectionsLoading/)
  assert.doesNotMatch(matchDetail, /\/api\/matches\/\$\{encodeURIComponent\(matchId\)\}\/connections/)
  assert.doesNotMatch(matchDetail, /target_phone|href=\{`tel:/)
...
test('home routes the Tonight CTA into the unified ranked journey', () => {
  const homePage = readSource('app/page.tsx')
  const homeLead = readSource('components/home/QuantumHomeLead.tsx')

  assert.match(homePage, /QuantumHomeLead/)
...
test('solo matching is blocked while a group flow is active', () => {
  ...
  assert.match(matchPage, /disabled=\{!devPreviewActive \|\| soloBlockedByGroupFlow\}/)
  assert.match(matchPage, /과팅 진행 중에는 소개팅을 잠시 막아둘게요/)
  assert.match(matchPage, /<MatchingPool[\s\S]+soloDisabled=\{!devPreviewActive \|\| soloBlockedByGroupFlow\}[\s\S]+groupQueueActive=\{isGroupQueueActive\}/)
...
test('solo introduction flow is separate from group creation', () => {
  ...
  assert.match(matchStartPage, /if \(steps\.every\(\(step\) => step\.done\) && appearanceScoreReady && mode === 'group' && !eventContext\) redirect\('\/group\/create'\)/)
})
...
test('solo queue and mock detail stay in the 1:1 flow', () => {
  ...
  assert.match(matchPage, /setSoloQueueActive\(nextMode === 'solo' && requestedSoloStatus === 'in_pool'\)/)
  assert.match(matchPage, /setMatches\(requestedSoloStatus === 'matched'[\s\S]+DEV_SOLO_MATCHES[\s\S]+DEV_MATCHES[\s\S]+: \[\]\)/)
  assert.match(matchPage, /setSoloQueueActive\(false\)/)
  assert.doesNotMatch(matchPage, /setSoloQueueActive\(requestedMode === 'solo' && params\.get\('soloStatus'\) === 'in_pool'\)/)
  assert.doesNotMatch(matchPage, /setMatches\(params\.get\('sampleMatches'\) === '1' \? DEV_SOLO_MATCHES : \[\]\)/)
  ...
  assert.doesNotMatch(matchDetailPage, /DEV_CONNECTIONS|createDevConnections\(matchId\)/)
  ...
  assert.match(matchDetailPage, /label: '채팅'/)
  assert.match(matchDetailPage, /상대의 작은 힌트/)
  assert.match(matchDetailPage, /전화번호는 자동으로 공개하지 않아요/)
  assert.match(matchDetailPage, /친구 요청을 서로 수락한 뒤 친구 메시지/)
  assert.match(matchDetailPage, /href="\/friends"/)
  assert.doesNotMatch(matchDetailPage, /target_phone|href=\{`tel:|자동 공개 예정/)
