...
test('community and meetup previews expose real routes without fake participation counts', () => {
  ...
  assert.match(meetupHub, /참여할 모임/)
  assert.match(community, /CommunityPortal/)
  assert.match(readSource('app/community/stories/page.tsx'), /communityCategoryCatalog/)
  assert.match(board, /\/api\/community\/posts/)
  assert.doesNotMatch(meetupHub + board, /가짜 통계|mock 참여/i)
})

test('community stories preserve board categories, guided writing, and a seven day hot destination', () => {
  const community = readSource('app/community/stories/page.tsx')
  const board = readSource('components/community/CommunityBoard.tsx')
...
test('meetup cylinder arrows stay outside the swipe capture surface and ideas open an activity-specific room lobby', () => {
  const hub = readSource('components/meetups/MeetupHub.tsx')
  const cylinder = readSource('components/meetups/MeetupIdeaCylinder.tsx')

  assert.match(cylinder, /data-swipe-surface="meetup-ideas"/)
  assert.match(cylinder, /\/meetups\/activities\/\$\{activeIdea\.id\}\/rooms\?gender_mode=\$\{genderMode\}/)
  assert.match(cylinder, /\{activeIdea\.title\} 모임방 보기/)
  assert.match(cylinder, /방마다 \{getMeetupCapacityRecommendation\(idea\.category\)\}명/)
  assert.match(cylinder, /방은 자동으로 준비돼요 · 정원 도달 시 다음 방 · 참여 후 채팅 약속/)
  assert.match(hub, /genderMode=\{genderFilter === 'any' \? 'all' : genderFilter\}/)
  assert.doesNotMatch(cylinder, /onBrowseCategory|내 시간으로 새 모임 열기|CirclePlus/)
  assert.doesNotMatch(hub, /browseIdeaCategory|openMeetupsRef/)
})

test('meetup cylinder treats the activity room lobby as the only selected-activity action', () => {
  const cylinder = readSource('components/meetups/MeetupIdeaCylinder.tsx')

  assert.match(cylinder, /Link/)
  assert.match(cylinder, /activeIdea\.id/)
  assert.match(cylinder, /genderMode/)
})

test('activity room links and direct creation keep an exact activity identity instead of only its database category', () => {
  const cylinder = readSource('components/meetups/MeetupIdeaCylinder.tsx')
  const createForm = readSource('components/meetups/CreateMeetupForm.tsx')
  const catalog = readSource('lib/community/catalog.ts')

  assert.match(catalog, /id:\s*'major-foundation-study'/)
  assert.match(cylinder, /\/meetups\/activities\/\$\{activeIdea\.id\}\/rooms/)
...
test('community migration denies direct table access and exposes bounded RPCs', () => {
  ...
  assert.match(meetupApi, /validateMeetupCreateV3Input/)
