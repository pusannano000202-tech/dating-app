...
test('profile API accepts only the fixed PNU self-selected scope through the atomic signup procedure', () => {
  const route = read('app/api/profile/basic/route.ts')

  assert.match(route, /schoolScope/)
  assert.match(route, /complete_minimum_signup/)
  assert.doesNotMatch(route, /school_change_requires_support/)
})
