...
test('Next development and production builds use separate output directories by default', () => {
  const nextConfig = readSource('next.config.mjs')

  assert.match(nextConfig, /const\s+defaultDistDir\s*=\s*isProduction\s*\?\s*['"]\.next['"]\s*:\s*['"]\.next-dev['"]/)
  assert.match(nextConfig, /distDir:\s*process\.env\.NEXT_DIST_DIR\s*\|\|\s*defaultDistDir/)
})
...
test('home page does not show result-like opponent card before matching starts', () => {
  ...
  assert.match(home, /QuantumHomeLead/)
  assert.match(today, /아직 신청한 매칭이 없어요/)
  assert.match(today, /href="\/tonight"/)
...
test('login page uses Supabase email OTP while phone provider is disabled', () => {
  ...
  assert.match(login, /getOAuthCallbackUrl\(appOrigin,\s*requestedRedirect\)/)
...
test('matching readiness gates include nickname and pre-match card checks', () => {
  ...
  assert.match(basicInfoForm, /\/api\/profile\/alias-options/)
  assert.match(basicInfoForm, /alias_ticket/)
  assert.match(basicInfoForm, /다른 이름 보기/)
  assert.match(basicInfoPage, /\/api\/profile\/basic/)
  assert.match(readSource('app/api/profile/basic/route.ts'), /complete_minimum_signup/)
...
test('minimum signup requires live auth while later legacy onboarding previews stay isolated', () => {
  const basic = readSource('app/profile/basic/page.tsx')
  const worldcup = readSource('app/profile/worldcup/page.tsx')
...
test('dev preview entry is gated by explicit development auth policy', () => {
  ...
  assert.match(middleware, /new URL\('\/login', appOrigin\)/)
