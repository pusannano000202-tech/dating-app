...
test('user journey includes exactly three photo ranks, secure friend invites, deposit, reveal, map and recovery', () => {
  ...
  assert.match(user, /at: data\.round\.depositDueAt, label: '보증금 마감'/)
  assert.match(user, /at: data\.round\.revealAt, label: '장소 공개'/)
  assert.match(user, /formatKoreanTime\(step\.at\)/)
