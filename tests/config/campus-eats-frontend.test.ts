...
test('campus eats pilot isolates its route and requires both production flags', () => {
  ...
  assert.match(route, /CampusEatsEntry/)
  assert.match(readSource('components/campus-eats/CampusEatsEntry.tsx'), /CampusEatsPilot/)
  assert.match(route, /notFound/)
  assert.match(route, /if \(!isCampusEatsFeatureEnabled\(\)\) notFound\(\)/)
  // The shared policy is behavior-tested for all production flag combinations.
  assert.match(route, /from '@\/lib\/community-feature'/)
...
test('campus eats setup selects every visited restaurant before the bracket begins', () => {
  ...
  assert.match(component, /\$\{category\.label\} 월드컵 시작/)
...
test('campus eats explains its visit-first battle with a replayable comic guide', () => {
  ...
  assert.match(pilot, /pendingDirectEntryRef.current.mode === 'battle'/)
  assert.match(pilot, /setView\(requiresBattleGuide \? 'map' : requestedView\)/)
  assert.match(pilot, /!battleGuideRequired/)
  assert.match(pilot, /setBattleGuideRequired\(requiresBattleGuide\)/)
  assert.match(pilot, /&& !restored.tournamentStarted/)
...
test('campus eats uses the official Naver map adapter with an honest missing-key state', () => {
  ...
  assert.match(map, /loadNaverMapsSdk/)
  assert.match(map, /subscribeNaverMapsAuthFailure/)
  assert.match(map, /authFailed/)
  ...
  assert.match(map, /new maps\.Marker/)
  assert.match(map, /fitBounds/)
  assert.match(map, /네이버 지도 연결이 필요해요/)
  assert.match(map, /role="status"/)
  assert.match(map, /aria-live="polite"/)
  assert.match(map, /ref=\{containerRef\}[\s\S]*?className="h-full w-full"/)
  assert.doesNotMatch(map, /ref=\{containerRef\}[\s\S]*?className="absolute inset-0"/)
  assert.match(map, /ResizeObserver/)
  assert.match(map, /maps\.Event\.trigger\?\.\(map, 'resize'\)/)
  assert.doesNotMatch(map, /navermap_authFailure\s*=/)
  assert.doesNotMatch(map, /leaflet|openstreetmap/i)
})

test('campus eats selected candidate and champion use the shared public place projection and accessible provider links', () => {
  const component = readSource('components/campus-eats/CampusEatsPilot.tsx')
  const mapView = component.slice(component.indexOf('function MapView'), component.indexOf('function ResultView'))
  const resultView = component.slice(component.indexOf('function ResultView'))

  assert.match(component, /toCampusEatsPublicPlace/)
  assert.match(component, /PlaceLinks/)
  assert.match(mapView, /toCampusEatsPublicPlace\(selectedCandidate,\s*\{/)
  assert.match(mapView, /schoolName:\s*school\.name/)
  assert.match(mapView, /<PlaceLinks place=\{selectedPlace\}/)
  assert.equal((mapView.match(/<PlaceLinks place=\{selectedPlace\}/g) ?? []).length, 2)
  assert.match(mapView, /!rankingOpen && selectedCandidate && selectedPlace/)
  assert.match(mapView, /aria-label=\{`\$\{selectedCandidate\.name\} 빠른 장소 안내`\}/)
  assert.match(mapView, /data-ui="selected-place-panel"/)
  assert.doesNotMatch(mapView, /data-ui="selected-place-panel"[\s\S]{0,250}className="[^"]*absolute/)
  assert.match(mapView, /data-ui="selected-place-panel"[\s\S]{0,350}shrink-0/)
  assert.match(resultView, /toCampusEatsPublicPlace\(winner,\s*\{/)
  assert.match(resultView, /schoolName:\s*school\.name/)
  assert.match(resultView, /<PlaceLinks place=\{winnerPlace\}/)
  assert.match(resultView, /우승 장소 위치 확인/)
  assert.match(component, /setSelectedCandidateId\(session\.winnerId \?\? null\)[\s\S]*?setRankingOpen\(false\)[\s\S]*?setView\('map'\)/)
  assert.doesNotMatch(mapView, /href=\{selectedCandidate\.naverSearchUrl\}/)
})

test('campus eats multi-pin map geocodes only address evidence as transient display coordinates', () => {
  const map = readSource('components/campus-eats/NaverCampusMap.tsx')
  const sdk = readSource('lib/places/naver-maps-sdk.ts')

  assert.match(map, /toCampusEatsPublicPlace/)
  assert.match(map, /schoolName/)
  assert.match(map, /\{\s*schoolName\s*\}/)
  assert.match(map, /\[apiKey,\s*candidates,\s*publishStatus,\s*retryRevision,\s*schoolName\]/)
  assert.match(map, /aria-label=\{`\$\{schoolName\} 캠퍼스 맛집 네이버 지도`\}/)
  assert.doesNotMatch(map, /aria-label="부산대 캠퍼스 맛집 네이버 지도"/)
  assert.match(map, /classifyCampusEatsMapResolution/)
  assert.match(map, /missing-addresses/)
  assert.match(map, /unresolved-addresses/)
  assert.match(map, /확인할 도로명 주소가 아직 없어요/)
  assert.match(map, /주소를 지도 핀으로 바꾸지 못했어요/)
  assert.doesNotMatch(map, /center:\s*new maps\.LatLng\(35\.2336,\s*129\.0796\)/)
  assert.match(map, /place\.address\?\.road/)
  assert.match(map, /data-coordinate-source="transient-address-geocode"/)
  assert.match(map, /주소 검색 기반 임시 핀/)
  assert.match(map, /response\?\.v2\?\.addresses/)
  assert.doesNotMatch(map, /coordinateCache\.set\(address,\s*null\)/)
  assert.match(sdk, /Status:\s*\{\s*OK:\s*number\s*\}/)
  assert.match(sdk, /response:\s*NaverGeocodeResponse\s*\|\s*undefined/)
  assert.doesNotMatch(map, /provider-verified/)
  assert.doesNotMatch(map, /localStorage|sessionStorage|fetch\(/)
})
...
test('campus eats map flow connects category tabs, a collapsible ranking rail, and collecting scores', () => {
  ...
  assert.match(component, /storageKey\(selectedSchool\.id, selectedCategory\.id\)/)
  assert.match(component, /const restoredView = resolveAutoView\([\s\S]*?restored\.tournamentStarted,\s*\)/)
...
test('campus eats battle advances a real bracket and separates champion from cumulative Elo ranking', () => {
  ...
  assert.match(result, /내 누적 취향 순위/)
  assert.match(result, /이번 월드컵 우승과는 다를 수/)
