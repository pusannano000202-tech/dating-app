...
test('social scenes launcher is fixed to local Supabase 56421 and Next 3013', () => {
  ...
  assert.match(source, /assertSocialScenesSchemaReady/)
  assert.match(source, /SOCIAL_SCENES_SCHEMA_PROBE_SQL/)
  assert.match(source, /'env', '-i'/)
  assert.match(source, /'-h', '\/var\/run\/postgresql'/)
  assert.ok(source.indexOf('const schemaPreflight =') < source.indexOf('const child = spawn('))
