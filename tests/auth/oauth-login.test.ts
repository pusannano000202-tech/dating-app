...
test('getOAuthCallbackUrl preserves the requested local destination', () => {
  ...
  assert.equal(
    getOAuthCallbackUrl('http://localhost:3004/', '/group/create?source=login'),
    'http://localhost:3004/auth/callback?next=%2Fgroup%2Fcreate%3Fsource%3Dlogin'
  )
  assert.equal(
    getOAuthCallbackUrl('https://dating-app-silk.vercel.app', 'https://evil.example/path'),
    'https://dating-app-silk.vercel.app/auth/callback'
  )
})
