...
export function normalizeCampusEatsSelectedRestaurantId(
  candidateIds: readonly string[],
  selectedRestaurantId: string | null,
): string | null {
  if (selectedRestaurantId && candidateIds.includes(selectedRestaurantId)) {
...
export function readCampusEatsUrlState(search: string): CampusEatsUrlState {
  ...
  if (category === 'coffee') {
    ...
    return {
      ...
      mode: mode === 'setup' ? 'setup' : mode === 'battle' ? 'battle' : 'map',
    }
  }
