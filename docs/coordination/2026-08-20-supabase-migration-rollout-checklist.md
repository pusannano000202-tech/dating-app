...
## 11. 2026-08-20 원격 권한 재감사와 내부 비밀키 강화

### 원격 Supabase 읽기 전용 확인

- 대상 프로젝트는 `quantum-production` (`jyfwcanjqwboyvicoafm`)이며 상태는
  `ACTIVE_HEALTHY`, PostgreSQL 17.6.1이다. 이번 감사에서는 SQL 조회와 Advisor
  확인만 했고 migration 적용이나 운영 데이터 수정은 하지 않았다.
- Security Advisor는 총 165건이다. RLS 정책 없는 테이블 41건, authenticated가
  실행 가능한 `SECURITY DEFINER` 함수 123건, 유출 비밀번호 보호 비활성화 1건이다.
- RLS 정책 없는 41개 테이블은 현재 `anon`과 `authenticated`의 직접 테이블 권한이
  각각 0건이다. 따라서 경고가 곧 공개 읽기·쓰기 취약점을 뜻하지는 않지만,
  향후 권한 부여 시 정책 없이 노출되지 않도록 계속 감시한다.
- authenticated 실행 가능 함수 123개 중 120개는 함수 본문에서 `auth.uid()`를
  직접 확인한다. 나머지 3개는 인증을 확인하는 내부 함수로만 위임한다.
- 함수 25개는 `search_path = public`을 사용하지만 모두 `auth.uid()`를 확인하고,
  `public` 스키마에 대한 PUBLIC/anon/authenticated의 `CREATE` 권한은 0건이다.
  즉시 악용 경로는 확인되지 않았지만 최소 권한 관점의 정리 대상이다.
- Performance Advisor는 인덱스 없는 외래 키 3건과 사용되지 않은 인덱스 35건이다.
  이는 보안 차단 항목은 아니며 실제 쿼리 사용량을 측정한 뒤 별도 최적화한다.

### API와 내부 서비스 인증 감사

- 상태 변경 API 86개를 분류했다. 사용자 경로는 `auth.getUser()`를 사용하고,
  예외 4개는 cron, 환불 작업자, 결제 취소 콜백, Toss webhook으로 각각 내부
  비밀키 또는 Toss 원격 결제 재조회 경계를 가진다.
- `PAYMENT_INTERNAL_SECRET`은 환불 작업자 런타임만 32자 이상을 요구하고 배포
  점검기와 결제 취소 경로는 더 짧은 값도 허용하던 불일치를 수정했다. 이제 모든
  경로가 32자 이상과 상수시간 비교를 사용한다.
- `CRON_SECRET`도 런타임과 배포 점검기가 모두 32자 이상을 요구하도록 통일했다.
- `AI_SERVER_SECRET`은 배포 점검기만 32자 이상을 요구하던 상태에서 Next.js와
  Python 런타임까지 같은 기준을 적용했다. 설정이 약하면 외모분석 요청을 보내지
  않고 안전한 구성 오류로 처리한다.
...
| 검증 | 결과 | 비고 |
| --- | --- | --- |
| 결제·cron 내부 인증 계약 | 통과 | 68/68, 31자 거부·32자 허용·상수시간 비교 확인 |
| 외모분석 계약 | 통과 | 24/24, Next.js·Python 32자 기준 확인 |
| Python 외모분석 API·시작 설정 | 통과 | 28/28, 격리된 Python 3.12 환경 |
| npm 전체 의존성 취약점 | 통과 | 개발 의존성 포함 0건 |
| Python 런타임 의존성 취약점 | 통과 | 27개 패키지, 알려진 취약점 0건 |
| 비밀정보 검사 | 통과 | 추적·미추적 파일에서 실제 키 문자열 0건 |

### 남은 운영 확인

- 이번 결과는 코드와 원격 권한의 읽기 전용 감사다. Vercel 운영 환경값이 없어서
  실제 배포 요청의 비밀키 일치와 키 교체는 아직 확인하지 않았다.
- 유출 비밀번호 보호는 현재 OTP·OAuth 로그인 구조에서 즉시 차단 사항은 아니지만,
