...
> **마지막 업데이트**: 2026-05-17 (Claude Code)  
> **다음 세션 작업자**: Codex  
...
### Claude Code 담당
1. **[대기 중] self_appearance_vector 타입 변경** — `lib/types.ts`의 `self_appearance_score: number` → `self_appearance_vector: SelfAppearanceVector`  
   ⚠️ Codex의 벡터 METADATA 작성 완료 후 진행, PR + 성준 리뷰 필요
2. **[대기 중] AppearanceSelfWorldcup.tsx 업데이트** — female-64/male-64 이미지 완성 후 교체
3. **Supabase 실키 연동** — 성준에게 URL/ANON_KEY 받으면 .env.local 세팅 후 E2E 테스트
4. **MatchingPool Supabase 실시간 구독** — 연동 후 작업
