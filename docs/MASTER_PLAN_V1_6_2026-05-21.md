...
### 12.G Male MI01-MI64 ✅ 완료 (2026-05-22, commit 32b44b2)

Manus + Codex 선별 결과:
- `public/appearance-ideal/male-64/` 에 MI01-MI96 96장 추가
- METADATA.json 에 male 96개 (active=64 + rejected=32) 등록, 모두 measured.appearance_vector 보유
- `public/appearance-ideal/MALE_FINAL_64_USAGE_SET.json` 으로 최종 64 선별 (final_bucket 분포 균형)
- `docs/handoff/MALE_IDEAL_WORLDCUP_MI01_MI96_CODEX_REVIEW_2026-05-22.md` 검토 문서
- `python/appearance/tests/test_ideal_worldcup_metadata.py` 회귀 테스트

검증:
- `selectActivePool(meta, 'male')` 가 64개 반환
- 여자 사용자의 이상형 월드컵 64카드 동작 가능

남은 운영 게이트:
- 외부 시각/API 스코어러 통과 (현재는 1st-pass score, 외부 ground-truth 검증 후 최종 active 확정 가능)
