...
- `[새]` z45 — **GPS 노쇼 자동 확정** (결정 8-24). checkin_attendance + finalize_no_show + batch_finalize_no_shows + Haversine + 알림 kind 2종 (attendance_confirmed, no_show_confirmed). 노쇼 발생 시 z41 자동 호출, z42 구걸 진입 차단.
- `e342907` /match/[id] GPS 패널 (체크인 / 노쇼 처리 버튼), API 3종 (checkin / finalize-no-show / attendance-state)
- `[새]` z46 — **Codex 리뷰 결함 수정** (결정 8-25). status 전이 + view security_invoker + admin/batch 권한 가드 + get_match_attendance_state 에 no_show_finalized/caller_is_no_show 노출. /match/[id] 가 노쇼 발생 시 continuation/refund/review CTA 자동 숨기고 안내 카드 노출
...
z45 gps_checkin_and_finalize_no_show   ← 후반: GPS 체크인 + 노쇼 자동 확정 + 구걸 차단 (결정 8-24)
z46 review_fixes_status_admin_view_guards ← 후반: Codex 리뷰 반영 (결정 8-25)
```
