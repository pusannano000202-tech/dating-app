...
e342907 z45 feat(gps) GPS 체크인 + 노쇼 자동 확정 + 알림 (결정 8-24)
e342907 feat(gps) /match/[id] GPS 패널 (체크인 / 노쇼 처리 버튼) + 알림 kind 2종 추가
[새] z46 fix Codex 리뷰 반영 (결정 8-25): status 전이 + view security_invoker + admin 가드 + UI 노쇼 분기
```
...
| 8-25 | **z45 결함 수정 (z46)** — Codex 리뷰 반영. (a) distribute_no_show_penalty 가 confirmed/completed 둘 다 허용 + service_role/admin/참여자 호출 허용 (b) finalize_no_show 안에서 status='confirmed' 면 즉시 completed 전이 (lazy +4h 무시) (c) finalize_no_show_admin/batch admin·service_role 가드 (d) admin_revenue_summary view security_invoker=on + WHERE is_admin() (e) get_match_attendance_state 가 no_show_finalized + caller_is_no_show 노출 → UI 가 노쇼 발생 시 continuation/refund/review CTA 자동 숨김. | 정책끼리 상태 충돌 + RLS 우회 + admin 함수 일반 노출 + UI 분기 미흡 모두 해결. 출시 차단 클래스. |
